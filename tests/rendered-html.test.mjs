...
test("server-renders the D2F Platform cockpit", async () => {
  ...
  assert.match(shell, /src="\/erp\/index\.html\?v=20260723-layout-v342"/);
...
test("ships a touch-first smartphone layout", async () => {
  ...
  assert.match(html, /styles\.css\?v=20260723-layout-v342/);
  assert.match(html, /app\.js\?v=20260723-layout-v342/);
...
test("makes the quote deposit unit explicit with no preset value", async () => {
  ...
  assert.match(platformVersion, /D2F_PLATFORM_VERSION = "3\.4\.2"/);
  assert.equal(JSON.parse(pkg).version, "3.4.2");
});
...
test("keeps every workspace scrollable and text contained on 13-inch screens", async () => {
  ...
  assert.match(html, /styles\.css\?v=20260723-layout-v342/);
  assert.match(shell, /erp\/index\.html\?v=20260723-layout-v342/);
});
