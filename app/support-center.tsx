...
export default function SupportCenter({ session, onClose, onAttentionCount, initialTicketId = "" }: { session: SupportSession; onClose: () => void; onAttentionCount: (count: number) => void; initialTicketId?: string }) {
  ...
  return <div className="support-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="support-center" role="dialog" aria-modal="true" aria-labelledby="support-title">
      <header className="support-head"><div><p className="eyebrow">D2F COMPLIANT · VERSION 2.1.4</p><h2 id="support-title">{payload?.isAdmin ? copy.adminTitle : copy.title}</h2><p>{payload?.isAdmin ? copy.adminSubtitle : copy.subtitle}</p></div><div><a href={`mailto:${payload?.supportEmail || "support@d2fcompliant.com"}`}><span>{copy.supportByEmail}</span><strong>{payload?.supportEmail || "support@d2fcompliant.com"}</strong></a><button type="button" onClick={onClose} aria-label={copy.close}>×</button></div></header>
