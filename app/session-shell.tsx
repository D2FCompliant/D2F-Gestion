    ...
    try {
      const result = await api("/auth/signup", {
        ...
        body: JSON.stringify({
          ...
          fullName: form.get("fullName"), email: form.get("email"), password, payerName: form.get("payerName"),
          acceptTerms: form.get("acceptTerms") === "on", website: form.get("website"),
        }),
      });
        ...
        <div className="auth-card">
          ...
          {mode === "login" ? (
            ...
            <form className="auth-form signup-form" onSubmit={submitSignup}>
              <div><p className="eyebrow">ABONNEMENT MENSUEL</p><h2>Créer votre espace</h2><p>Une entreprise, deux utilisateurs. Activation après validation du virement bancaire.</p></div>
              <div className="form-grid">
                ...
                <label className="span-2">Titulaire du virement<input name="payerName" placeholder="Nom figurant sur le virement" required /></label>
...
function AccountDrawer({ session, onSession, onClose }: { session: SessionData; onSession: (value: SessionData) => void; onClose: () => void }) {
  ...
  async function declarePayment(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      const updated = await api("/auth/subscription", { method: "POST", body: JSON.stringify({ payerName: form.get("payerName"), transferReference: form.get("transferReference"), paidOn: form.get("paidOn") }) });
...
function LockedSubscription({ session, onOpen }: { session: SessionData; onOpen: () => void }) {
  return <main className="locked-page"><img src="/d2f-gestion-logo.png" alt="D2F Gestion" /><p className="eyebrow">ESPACE CRÉÉ</p><h1>{formatStatus(session.account.status)}</h1><p>Votre entreprise est enregistrée. L’application sera ouverte après validation du virement par D2F Compliant.</p><div className="reference-card"><span>Référence à indiquer</span><strong>{session.account.subscription.bankTransferReference || "D2F vous communiquera la référence"}</strong></div><button className="primary-action" onClick={onOpen}>Voir l’abonnement et déclarer le virement</button></main>;
}
...
export default function SessionShell() {
  ...
  return <main className="app-session-shell">
    ...
    {session.account.canUseApplication ? <iframe className="web-app-frame" src="/erp/index.html?v=20260716-saas-v1" title="D2F Gestion" allow="clipboard-read; clipboard-write" /> : <LockedSubscription session={session} onOpen={() => setDrawer(true)} />}
